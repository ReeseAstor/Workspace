...
2. Copy and edit environment variables
   ...
   cp .env.example .env
   ```
   Set both `LANDING_PAGE_USERNAME` and `LANDING_PAGE_PASSWORD` if you want the dashboard and API protected by HTTP Basic authentication.
...
| `LANDING_PAGE_USERNAME` / `LANDING_PAGE_PASSWORD` | Optional HTTP Basic Auth credentials for the dashboard and API. When both are set, all routes except `/health` require authentication. |
